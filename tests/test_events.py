...
def test_ids_are_unique_and_run_is_stable_within_a_factory() -> None:
    factory = EventFactory(source="naust://h/worlds/w")
    a = factory.cloudevent(Event("x", {}), 1)
    b = factory.cloudevent(Event("x", {}), 2)
    assert a["id"] != b["id"]
    assert a["naustrun"] == b["naustrun"]
    assert EventFactory(source="naust://h/worlds/w").run != factory.run
